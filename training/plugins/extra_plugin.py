...
class ExtraPlugin(SupervisedPlugin):
    """
    ...
    def before_training_exp(self, strategy: "BaseStrategy",
        ...
        if self.task_balanced_dataloader:
            num_keys = len(self.storage_policy.buffer_groups.keys())
            current_batch_size = batch_size_mem // num_keys
            remaining_example = batch_size_mem % num_keys

            self.loaders_dict = {}
            self.iter_buffer_dataloaders = {}
            for class_id in self.storage_policy.buffer_groups.keys():
                data = self.storage_policy.buffer_groups[class_id].buffer
                scores = self.storage_policy.buffer_groups[class_id]._buffer_weights

                data = GetScores(data, scores)
                ...
                self.loaders_dict[class_id] = DataLoader(
                                GetScores(data, scores), batch_size=current_batch_size, shuffle=True, **kwargs
                        )
                self.iter_buffer_dataloaders[class_id] = iter(self.loaders_dict[class_id])
        else:
            datasets = []
            scores = []
            for class_id in self.storage_policy.buffer_groups.keys():
    ...
    def iter_buffer(self):
        if self.task_balanced_dataloader:
            batch = []
            for class_id, dataloader in self.iter_buffer_dataloaders.items():
                try:
                    batch.append(next(dataloader))
                except StopIteration:
                    self.iter_buffer_dataloaders[class_id] = iter(self.loaders_dict[class_id])
                    batch.append(next(self.iter_buffer_dataloaders[class_id]))

            tbatch = _default_collate_mbatches_fn(batch)
        else:
            try:
